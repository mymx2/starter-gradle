...
export const fns = [patternMatchDemo, globMatchDemo]

// ---------------------------------------------------------------------------
// 参考代码（不参与运行）：GitHub Actions 各 workflow 事件的 context 用法示例。
// 依赖的 import：
//   import type { IssueCommentEvent, IssuesLabeledEvent, PullRequestOpenedEvent, WorkflowRunCompletedEvent } from '@octokit/webhooks-definitions/schema.d.ts'
...
//   console.log(workflowRun.workflow.name, workflowRun.workflow_run.id)
//   const workflow = github.workflow
//   if (workflow === 'Issue Labeled') {
//     const event = github.event.issues as IssuesLabeledEvent
//     console.log(event.label?.name, event.issue.number, event.issue.user.login)
//   }
...
//       console.log(event.comment.body, event.issue.pull_request, event.comment.author_association)
//     }
//   }
